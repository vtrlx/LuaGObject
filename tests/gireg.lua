-- LuaGObject test suite using GI Regress tests.
...
function gireg.type_int64()
	...
	-- These tests fail on versions of Lua before 5.3 as their only numeric type is double-precision floating point, which cannot represent 64-bit integers. Lua 5.3 and later run these tests without issue.
	if nativeIntegers then
		...
		-- These two tests have been disabled because they are inconsistent with expected behaviour.
...
function gireg.boxed_a_equals()
	local R = LuaGObject.Regress
	check(R.TestSimpleBoxedA({
			some_int = 1,
			some_int8 = 2,
			some_double = 3.14,
		}):equals(R.TestSimpleBoxedA({
			some_int = 1,
			some_int8 = 2,
			some_double = 3.14,
		})))
	check(not R.TestSimpleBoxedA({
			some_int = 2,
			some_int8 = 2,
			some_double = 3.14,
		}):equals(R.TestSimpleBoxedA({
...
function gireg.obj_prop_dynamic()
	...
	-- Remove static property information, forcing LuaGObject to use GLib's dynamic property system.
