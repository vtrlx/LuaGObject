...
-- Lua 5.3 and 5.1 seems to have some problem with aggressive GC settings used here and completing this test takes literally ages, so skip it. Enable only if explicitely asked for.
...
function gobject.subclass_prop_inherit()
	...
	-- Gio.Initable is invoked automatically by LuaGObject after object construction.
...
-- Test that the manual :connect_signal() method works, including its optional detail.
function gobject.manual_signal_connect()
	local notified = false
	local GObject = LuaGObject.GObject
	local Derived = GObject.Object:derive("LgiTestDerivedConnectSignal")
	Derived._property.num = GObject.ParamSpecDouble(
		"num", "Number", "Test number", 0, 1, 0,
		{ "READABLE", "WRITABLE", "CONSTRUCT" })
	Derived._property.str = GObject.ParamSpecString(
		"str", "Nick string", "Blurb string", "string-default",
		{ "READABLE", "WRITABLE", "CONSTRUCT" })
	...
	local der = Derived()
	der:connect_signal("notify", function()
		notified = true
	end, "str")
	der.num = 1
	check(not notified)
