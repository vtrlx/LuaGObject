...
-- Implementation of record_mt, which is inherited from component and provides customizations for structures and unions.
local record = {
...
function record.struct_mt:_access_field(instance, element, ...)
	...
	if select('#', ...) > 0 then
		if gi.isinfo(element) and element.is_field then
			...
			if type(element) == 'table' and
					(element[2] == 1 or element[2] == 2) then
				subrecord = true
			end
		end
	end
	...
	if subrecord then
		-- Write to nested structure, handle assignment to it by assigning separate fields.
...
function record.load(info)
	...
	-- Check, whether global namespace contains 'constructor' method, i.e. method which has the same name as our record type (except that type is in CamelCase, while method is under_score_delimited).  If not found, check for 'new' method.
	...
	-- Check, whether ctor is valid.  In order to be valid, it must return instance of this record.
...
-- Union metatable is the same as struct one, but has different name to differentiate unions.
