...
-- Table containing loaders for various GI types, indexed by gi.InfoType constants.
local typeloader = {}
...
local namespace = {
	mt = {
		_categories = {
			'_class', '_interface', '_struct', '_union', '_enum', '_function', '_constant',
		},
		_category_mt = {},
	}
}
...
function namespace.mt:__index(symbol)
	...
	if loader then
		...
		if val then
			...
			-- Store symbol into the repo, but only if it is not already there.  It could by added to repo as byproduct of loading other symbol.
			if not cat[symbol] then cat[symbol] = val end
		elseif info.is_gtype_struct then
			-- If we have XxxClass name, try to lookup class structure of the Xxx object.
...
function namespace.mt:_resolve(recurse)
	-- Iterate through all items in the namespace and dereference them, which causes them to be loaded in and cached inside the namespace table.
...
-- Makes sure that the namespace (optionally with requested version) is properly loaded.
function namespace.require(name, version)
	-- Load the namespace info for GIRepository.  This also verifies whether requested version can be loaded.
	...
	if not ns then
		...
		if not ok then
			-- Try parsing message; if it is something different than "module xxx not found", then attempt to load again and let the exception fly out.
		...
		if ok and type(msg) == "string" then
			-- It's an error message for something that does not want to be re-loaded, see e.g. the call to Gtk.init_check() in Gtk.lua.
			error(msg)
		end
	end
	return ns
