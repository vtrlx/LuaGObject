--  LGI Support for repository packages (namespaces with classes overriden in LuaGObject)
...
-- There is no lazy-loading, but define _resolve to do nothing to achieve API compatibility with GI-based namespaces.
function package.mt:_resolve(recurse)
	return self
end

-- Defines new class, deriving from existing one.  If the class already exists, does nothing and returns nil, otherwise returns newly created class type.
