...
-- Generic component metatable.  Component is any entity in the repo, e.g. record, object, enum, etc.
local component = { mt = {} }

-- Creates new component table by cloning all contents and setting Gets table for category of compound (i.e. _field of struct or _property for class etc).  Installs metatable which performs on-demand lookup of symbols.
function component.get_category(children, xform_value,
	...
	-- Index contains array of indices which were still not retrieved from 'children' table, and table part contains name->index mapping.
	local index, mt = {}, {}
	for i = 1, #children do index[i] = i end

	-- Fully resolves the category (i.e. loads everything remaining to be loaded in given category) and disconnects on-demand loading metatable.
	local function resolve(category)
		...
		for en, idx in pairs(index) do
			...
			local name = not xform_name_reverse and en
				or xform_name_reverse(en)
	...
	function mt:__index(requested_name)
		...
		if idx then
			...
			-- Not yet, go through unknown indices and try to find the name.
			while #index > 0 do
		...
		-- If there is nothing in the index, we can disconnect metatable, because everything is already loaded.
		if not next(index) then
			setmetatable(self, nil)
		end

		-- Transform found value and store it into the category (self) table.
...
-- Creates new component table by cloning all contents and setting categories table.
...
-- Fully resolves the whole typetable, i.e. load all symbols normally oaded on-demand at once.  Returns self, so that resolve can be easily chained for the caller.
...
-- Implementation of _access method, which is called by _core when instance of repo-based type is accessed for reading or writing.
function component.mt:_access(instance, symbol, ...)
	...
	-- If specific accessor does not exist, consider the element to be 'static const' attribute of the class.  This works well for methods, constants and assorted other elements added manually into the class by overrides.
	if select('#', ...) > 0 then
...
-- Keyword translation dictionary.  Used for translating Lua keywords which might appear as symbols in typelibs into Lua-neutral identifiers.
...
-- Retrieves (element, category) pair from given componenttable and instance for given symbol.
function component.mt:_element(instance, symbol, origin)
	-- This generic version can work only with strings.  Refuse everything other, hoping that some more specialized _element implementation will handle it.
	if type(symbol) ~= 'string' then return end

	-- Check keyword translation dictionary.  If the symbol can be found there, try to lookup translated symbol.
	...
	-- Check whether symbol is accessible in cached directory of the component, packed as element value and category
	...
	-- Decompose symbol name, in case that it contains category prefix (e.g. '_field_name' when requesting explicitely field called name).
	...
	if element then
		-- Make sure that table-based attributes have symbol name, so that potential errors contain the name of referenced attribute.
		...
		if not category or not (origin or self)['_access' .. category] then
			-- No category or no special category handler is present, store it directly, which results in fastest access.  This is most typical for methods.
			self[symbol] = element
		else
			-- Store into _cached table, because we have to preserve the category.
			if not cached then
				cached = {}
				self._cached = cached
			end
...
function component.mt:__index(key)
	...
	if value then
		if category then
			-- Mangle the result by type-specific '_index_<category>' method, if present.
...
-- Implementation of attribute accessor.  Attribute is either function to be directly invoked, or table containing set and get functions.
function component.mt:_access_attribute(instance, element, ...)
	-- If element is a table, assume that this table contains 'get' and 'set' methods.  Dispatch to them, and error out if they are missing.
...
-- Creates new component and sets up common parts according to given info.
