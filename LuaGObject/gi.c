...
/* Userdata representing single group of infos(e.g. methods on object, fields of struct etc.). Emulates Lua table for access. */
	...
	if (GI_IS_REGISTERED_TYPE_INFO(*info)) {
		if (strcmp(prop, "gtype") == 0) {
			...
			if (strcmp(prop, "is_gtype_struct") == 0) {
				lua_pushboolean(L,
					gi_struct_info_is_gtype_struct(GI_STRUCT_INFO(*info)));
				return 1;
			} else if (strcmp(prop, "size") == 0) {
				lua_pushinteger(L,
					gi_struct_info_get_size(GI_STRUCT_INFO(*info)));
				return 1;
			}
			INFOS(struct, field)
			INFOS(struct, method);
			...
			if (strcmp(prop, "size") == 0) {
				lua_pushinteger(L,
					gi_union_info_get_size(GI_UNION_INFO(*info)));
				return 1;
			}
			INFOS(union, field)
			INFOS(union, method);
			...
			if (strcmp(prop, "type_struct") == 0)
				return lua_gobject_gi_info_new(L,
					GI_BASE_INFO(gi_interface_info_get_iface_struct(
			...
			if (strcmp(prop, "parent") == 0)
				return lua_gobject_gi_info_new(L, GI_BASE_INFO(
						gi_object_info_get_parent(GI_OBJECT_INFO(*info))));
		...
		if (strcmp(prop, "return_type") == 0)
			return lua_gobject_gi_info_new(L, GI_BASE_INFO(
					gi_callable_info_get_return_type(
						GI_CALLABLE_INFO(*info))));
		else if (strcmp(prop, "return_transfer") == 0)
			return info_push_transfer(L, gi_callable_info_get_caller_owns(
					GI_CALLABLE_INFO(*info)));
		INFOS(callable, arg);

		if (GI_IS_SIGNAL_INFO(*info)) {
			if (strcmp(prop, "flags") == 0) {
				GSignalFlags flags = gi_signal_info_get_flags(
					GI_SIGNAL_INFO(*info));
				lua_newtable(L);
#define H(n1, n2) \
...
#undef H
		...
		if (GI_IS_FUNCTION_INFO(*info)) {
			if (strcmp(prop, "flags") == 0) {
				GIFunctionInfoFlags flags = gi_function_info_get_flags(
					GI_FUNCTION_INFO(*info));
				lua_newtable(L);
				if (0);
#define H(n1, n2) \
...
#undef H
	...
	if (GI_IS_ENUM_INFO(*info) || GI_IS_FLAGS_INFO(*info)) {
		if (strcmp(prop, "storage") == 0) {
			GITypeTag tag = gi_enum_info_get_storage_type(
				GI_ENUM_INFO(*info));
		...
		INFOS(enum, value)
			...
			const gchar *domain = gi_enum_info_get_error_domain(
				GI_ENUM_INFO(*info));
			if (domain != NULL)
	...
	if (GI_IS_ARG_INFO(*info)) {
		if (strcmp(prop, "direction") == 0) {
			...
			if (dir == GI_DIRECTION_OUT)
				lua_pushstring(L,
					gi_arg_info_is_caller_allocates(GI_ARG_INFO(*info))
	...
	if (GI_IS_PROPERTY_INFO(*info)) {
		if (strcmp(prop, "flags") == 0) {
			lua_pushinteger(L,
				gi_property_info_get_flags(GI_PROPERTY_INFO(*info)));
			return 1;
		}
		else if (strcmp(prop, "transfer") == 0)
			return info_push_transfer(L,
				gi_property_info_get_ownership_transfer(
					GI_PROPERTY_INFO(*info)));
	}
	...
	if (GI_IS_TYPE_INFO(*info)) {
		...
		if (strcmp(prop, "tag") == 0) {
			...
			if (tag == GI_TYPE_TAG_ARRAY || tag == GI_TYPE_TAG_GLIST
				...
				lua_newtable(L);
				lua_gobject_gi_info_new(L,
					GI_BASE_INFO(gi_type_info_get_param_type(
						GI_TYPE_INFO(*info), 0)));
				lua_rawseti(L, -2, 1);
				if (tag == GI_TYPE_TAG_GHASH) {
					lua_gobject_gi_info_new(L, GI_BASE_INFO(
			...
			lua_gobject_gi_info_new(L, gi_type_info_get_interface(
					GI_TYPE_INFO(*info)));
			return 1;
...
#undef H
			...
			lua_pushboolean(L, gi_type_info_is_zero_terminated(
				GI_TYPE_INFO(*info)));
			return 1;
...
namespace_len(lua_State *L)
{
	...
	lua_pushinteger(L, gi_repository_get_n_infos(
		lua_gobject_gi_get_repository(), ns));
	return 1;
}
...
namespace_index(lua_State *L)
{
	...
	const gchar *prop;
	if (lua_type(L, 2) == LUA_TNUMBER) {
		GIBaseInfo *info = gi_repository_get_info(
			lua_gobject_gi_get_repository(), ns, lua_tointeger(L, 2) - 1);
		return lua_gobject_gi_info_new(L, info);
	}
	prop = luaL_checkstring(L, 2);
	if (strcmp(prop, "dependencies") == 0) {
		gchar **deps = gi_repository_get_dependencies(
			lua_gobject_gi_get_repository(), ns, NULL);
		if (deps == NULL)
			lua_pushnil(L);
		else {
			int index;
		...
		lua_pushstring(L, gi_repository_get_version(
			lua_gobject_gi_get_repository(), ns));
		return 1;
		...
		lua_setmetatable(L, -2);
		*udata = gi_repository_require(lua_gobject_gi_get_repository(),
			ns, NULL, 0, NULL);
		return 1;
	}
	else
		...
		return lua_gobject_gi_info_new(L,
			gi_repository_find_by_name(
				lua_gobject_gi_get_repository(), ns, prop));
}
...
gi_require(lua_State *L)
{
	...
	if (typelib_dir == NULL)
		typelib = gi_repository_require(lua_gobject_gi_get_repository(),
			namespace, version, 0, &err);
	else
		typelib = gi_repository_require_private(lua_gobject_gi_get_repository(),
			typelib_dir, namespace, version, 0, &err);
	if (!typelib) {
		lua_pushboolean(L, 0);
...
gi_index(lua_State *L)
{
	if (lua_type(L, 2) == LUA_TLIGHTUSERDATA) {
		...
		GIBaseInfo *info = GI_BASE_INFO(gi_repository_find_by_error_domain(
			lua_gobject_gi_get_repository(), domain));
		return lua_gobject_gi_info_new(L, info);
	} else {
...
#if !GLIB_CHECK_VERSION(2, 30, 0)
/* Workaround for broken gi_struct_info_get_size() for GValue, see https://bugzilla.gnome.org/show_bug.cgi?id=657040 */
...
lua_gobject_struct_info_get_size(GIStructInfo *info)
{
	if (parameter_info == NULL)
		parameter_info = gi_repository_find_by_name(
