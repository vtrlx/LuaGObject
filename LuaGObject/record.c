...
typedef enum _RecordStore {
	...
	/* Record is placed inside some other(parent) record. In order to keep parent record alive, parent record is stored in parent_cache table. */
	RECORD_STORE_NESTED,

	/* Record is allocated by its GLib means and must be freed(by g_boxed_free). */
	RECORD_STORE_ALLOCATED,
	} RecordStore;

/* Userdata containing record reference. Table with record type is attached as userdata environment. */
typedef struct _Record {
	...
	/* If the record is allocated 'on the stack', its data is here. Anonymous union makes sure that data is properly aligned to hold(hopefully) any structure. */
	union {
	gchar data[1];
	double align_double;
	long align_long;
...
/* lightuserdata key to LUA_REGISTRYINDEX containing metatable for record. */
static int record_mt;

/* lightuserdata key to cache table containing lightuserdata(record->addr) -> weak(record) */
static int record_cache;

/* lightuserdata key to cache table containing recordproxy(weak) -> parent */
...
lua_gobject_record_new(lua_State *L, int count, gboolean alloc)
{
	...
	/* Allocate new userdata for record object, attach proper metatable. */
	record = lua_newuserdata(L,
		G_STRUCT_OFFSET(Record, data) +(alloc ? 0 : size));
...
lua_gobject_record_2lua(lua_State *L, gpointer addr, gboolean own, int parent)
{
	...
	record->addr = addr;
	if (parent != 0) {
		/* Store reference to the parent argument into parent reference cache. */
		...
		if (!own) {
			/* Check, whether refrepo table specifies custom _refsink function. */
	...
	/* Assign refrepo table(on the stack when we are called) as environment for our proxy. */
	lua_pushvalue(L, -4);
	lua_setfenv(L, -2);
	...
	/* Clean up the stack; remove cache table from under our result, and remove also typetable which was present when we were called. */
	lua_replace(L, -4);
	lua_pop(L, 2);
}

/* Checks that given argument is Record userdata and returns pointer to it. Returns NULL if narg has bad type. */
static Record *
	...
	gboolean own, gboolean optional, gboolean nothrow)
{
	...
	if (!optional || !lua_isnoneornil(L, narg)) {
		...
		if (record) {
			/* Check, whether type fits. Also take into account possible inheritance. */
			lua_getfenv(L, narg);
			for (;;) {
				if (lua_equal(L, -1, -2))
					break;
...
record_gc(lua_State *L)
{
	...
	if (record->store == RECORD_STORE_EMBEDDED
			|| record->store == RECORD_STORE_NESTED) {
		/* Check whether record has registered '_uninit' function, and invoke it if yes. */
		lua_getfenv(L, 1);
...
/* Implements generic record creation. Creates new record instance, unless 'addr' argument(lightuserdata or integer) is specified, in which case wraps specified address as record. Lua prototype:
recordinstance = core.record.new(repotable[, nil[, count[, alloc]]])
recordinstance = core.record.new(repotable[, addr[, own]])

own(default false) means whether Lua takes record ownership (i.e. if it tries to deallocate the record when created Lua proxy dies). */
static int
...
- 'addr': returns address of the object. If 3rd argument is either repotable, checks, whether record conforms to the specs and if not, throws an error. */
static int
...
/* Assumes that given record is the first of the array of records and fetches record with specified index. Negative indices are allowed, but no bounds checking is attempted.
...
record_fromarray(lua_State *L)
{
	...
	lua_getfenv(L, 1);
	lua_gobject_record_2lua(L, ((guint8 *) record->addr) + size * index,
		own, parent);
	return 1;
}
...
- 'own' if true, changing ownership to owned, otherwise to unowned.
record.set(recordinstance, repotable)
- 'repotable' record repotable to which this instance should be reassigned. */
static int
...
/* Workaround for g_value_unset(), which complains when invoked on uninitialized GValue instance. */
static void
...
/* Similar stuff for g_value_copy(), requires target argument to be preinitialized with proper type. */
static void
