...
function enum.load(info, meta)
	...
	if info.methods then
		...
		-- Enum.methods was added only in GI1.30; for older gi, simulate the access using lookup in the global namespace.
	...
	-- Install metatable providing reverse lookup (i.e name(s) by value).
	return enum_type
end
...
-- Constructs enum number from specified string. Eventually, for error-type enums, allows creating error instance.
...
-- Resolving arbitrary number to the table containing symbolic names of contained bits.
