...
-- Object constructor, 'param' contains table _with properties/signals to initialize.
local parameter_repo = repo.GObject.Parameter
-- Before GLib 2.54, g_object_newv() was annotated with [rename-to g_object_new]. Starting from GLib 2.54, g_object_new_with_properties() has this annotation. We always want g_object_newv().
local object_new = gi.GObject.Object.methods.newv or gi.GObject.Object.methods.new
if object_new then
	object_new = core.callable.new(object_new)
else
	-- Unfortunately, older GI (<1.30) does not export g_object_newv() in the typelib, so we have to workaround here with manually implemented C version.
	...
	for name, arg in pairs(param or {}) do
		if type(name) == 'string' then
			...
			if gi.isinfo(argtype) and argtype.is_property then
				...
				-- Store the name string in some safe Lua place ('safe' table), because param is GParameter, which contains only non-owning pointer to the string, and it could be Lua-GC'ed while still referenced by GParameter instance.
				safe[#safe + 1] = atname
				safe[#safe + 1] = value
...
-- Override normal 'new' constructor, to allow creating objects with specified GType.
...
-- Initially unowned creation is similar to normal GObject creation, but we have to ref_sink newly created object.
...
function Object:_access_signal(object, info, ...)
	...
	if select('#', ...) > 0 then
		...
		-- If signal supports details, add metatable implementing __newindex for connecting in the 'on_signal['detail'] = handler' form.
...
-- For certain libraries, type info is incomplete and signals are not found by introspection. In these cases, it's useful to provide a good helper method
function Object:connect_signal(name, callback, detail, after)
	local info = gi[core.gtype(self._gtype)]
	local closure = Closure((callback), info)
	return signal_connect_closure_by_id(
		self, signal_lookup(name, self._gtype),
		detail and quark_from_string(detail) or 0, closure, after or false)
end

-- GOI<1.30 does not export 'Object.on_notify' signal from the typelib. Work-around this problem by implementing custom on_notify attribute.
...
-- Bind property implementation. For some strange reason, GoI<1.30 exports it only on GInitiallyUnowned and not on GObject. Oh well...
