...
-- Implementation of field accessor.  Note that compound fields are not supported in classes (because they are not seen in the wild and I'm lazy).
...
-- Add __index for _virtual handling.  Convert vfi baseinfo into real callable pointer according to the target type.
...
function class.load_class(namespace, info)
	...
	-- Create class instance, copy mt from parent, if parent exists, otherwise defaults to class_mt.
...
function class.class_mt:derive(typename, ifaces)
	...
	-- Create class-initialization closure, which assigns pointers to all known overriden virtual methods.
...
-- Overload __newindex to catch assignment to virtual - this causes installation of new virtual method
function class.derived_mt:__newindex(name, target)
	...
	if category == '_virtual' then
		-- Overriding virtual method. Prepare callback to the target and store it to the _override type helper subtable.
		...
		-- Simply assign to type.  This most probably means adding new member function to the class (or some static member).
