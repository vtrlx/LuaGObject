...
/* Checks whether given argument contains number which fits given constraints. If yes, returns it, otherwise throws Lua error. */
...
/* Marshals integral types to C.  If requested, makes sure that the value is actually marshalled into val->v_pointer no matter what the input type is. */
static void
	...
	if (gi_type_info_get_array_length_index(ti, &param) && ci != NULL) {
		...
		if (GI_IS_FUNCTION_INFO(ci) || GI_IS_CALLBACK_INFO(ci)) {
			...
			if (gi_arg_info_get_direction(ai) == GI_DIRECTION_IN)
				/* For input parameters, value is directly pointed to by args table element. */
				val = (GIArgument *)((void **) args)[param];
			else
				/* For output arguments, args table element points to pointer to value. */
				val = *(GIArgument **)((void **) args)[param];
...
/* Retrieves pointer to GIArgument in given array, given that array contains elements of type ti. */
static gssize
	...
	gboolean optional, GITransfer transfer)
{
	...
	if (optional && lua_isnoneornil(L, narg)) {
		...
		*out_array = NULL;
		...
		/* Check the type. If this is C-array of byte-sized elements, we can try special-case and accept strings or buffers. */
		*out_array = NULL;
		if (lua_type(L, narg) != LUA_TTABLE && esize == 1
				&& atype == GI_ARRAY_TYPE_C) {
			size_t size = 0;
			*out_array = lua_gobject_udata_test(L, narg,
				LUA_GOBJECT_BYTES_BUFFER);
			if (*out_array)
		...
		if (!*out_array) {
			...
			if (atype != GI_ARRAY_TYPE_C
				|| !gi_type_info_get_array_fixed_size(ti,(gsize *)out_size))
				*out_size = objlen;
			...
			/* Return either GArray or direct pointer to the data, according to the array type. */
			if (array == NULL)
				*out_array = NULL;
			else
				switch (atype) {
		...
		gpointer array, gssize size, int parent)
{
	...
	if (atype == GI_ARRAY_TYPE_ARRAY) {
		...
		if (gi_type_info_is_zero_terminated(ti))
			...
			if (!gi_type_info_get_array_fixed_size(ti,(gsize *)&len))
				/* Length of the array is dynamic, get it from other argument. If the size isn't known ahead of time(it's -1), and it isn't zero-terminated, assume 1. */
				len = size < 0 ? 1 : size;
		}
	}

	/* Get array element type info, wrap it in the guard so that we don't leak it. */
	eti = gi_type_info_get_param_type(ti, 0);
	lua_gobject_gi_info_new(L, GI_BASE_INFO(eti));
	eti_guard = lua_gettop(L);
	esize = array_get_elt_size(eti, atype == GI_ARRAY_TYPE_PTR_ARRAY);

	/* Note that we ignore is_pointer check for uint8 type. Although it is not exactly correct, we probably would not handle uint8* correctly anyway, this is strange type to use, and moreover this is workaround for g-ir-scanner bug which might mark elements of uint8 arrays as gconstpointer, thus setting is_pointer=true on it. See https://github.com/lgi-devs/lgi/issues/57 */
	if (gi_type_info_get_tag(eti) == GI_TYPE_TAG_UINT8) {
		...
		if (array == NULL) {
			/* NULL array is represented by empty table for C arrays, nil for other types. */
			if (atype == GI_ARRAY_TYPE_C)
				lua_newtable(L);
			else
				lua_pushnil(L);
		...
		for (index = 0; len < 0 || index < len; index++) {
			...
			/* If the array is zero-terminated, terminate now and don't include NULL entry. */
...
/* Marshalls GSList or GList from Lua to C. Returns number of temporary elements pushed to the stack. */
static int
		...
		gpointer *list, int narg, GITransfer transfer)
{
	...
	/* Allow empty list to be expressed also as 'nil', because in C, there is no difference between NULL and empty list. */
	...
	/* Get list element type info, create guard for it so that we don't leak it. */
	eti = gi_type_info_get_param_type(ti, 0);
	lua_gobject_gi_info_new(L, GI_BASE_INFO(eti));
	eti_guard = lua_gettop(L);

	/* Go from back and prepend to the list, which is cheaper than appending. */
	guard = (GSList **) lua_gobject_guard_create(L,
		list_tag == GI_TYPE_TAG_GSLIST
		? (GDestroyNotify) g_slist_free
		: (GDestroyNotify) g_list_free);
	while (index > 0) {
		/* Retrieve index-th element from the source table and marshall it as pointer to arg. */
		GIArgument eval;
...
/* Marshalls hashtable from Lua to C. Returns number of temporary elements pushed to the stack. */
static int
		...
		gboolean optional, GITransfer transfer)
{
	...
	if (optional && lua_isnoneornil(L, narg))
		*table = NULL;
		...
		/* Create the hashtable and guard it so that it is destroyed in case something goes wrong during marshalling. */
		guarded_table = (GHashTable **) lua_gobject_guard_create(L,(GDestroyNotify) g_hash_table_destroy);
	vals++;

		/* Find out which hash_func and equal_func should be used, according to the type of the key. */
		...
		case GI_TYPE_TAG_DOUBLE:
			return luaL_error(L, "hashtable with float or double is not " "supported");
		default:
		...
		*guarded_table = *table = g_hash_table_new(hash_func, equal_func);
		...
		while (lua_next(L, narg)) {
			...
			/* The great stack suffle; remove value completely and leave key on the top of the stack. Complicated by the fact that both are burried under key_pop + val_pop elements created by marshalling. */
		...
		GITransfer xfer, GHashTable *hash_table)
{
	...
	if (hash_table == NULL)
		...
		/* Get key and value type infos, guard them so that we don't leak it. */
		guard = lua_gettop(L) + 1;
		for (i = 0; i < 2; i++) {
			eti[i] = gi_type_info_get_param_type(ti, i);
			lua_gobject_gi_info_new(L, GI_BASE_INFO(eti[i]));
		...
		while (g_hash_table_iter_next(&iter, &eval[0].v_pointer, &eval[1].v_pointer)) {
			...
			for (i = 0; i < 2; i++)
				lua_gobject_marshal_2lua(L, eti[i], NULL, dir,
					GI_TRANSFER_NOTHING, &eval[i],
					LUA_GOBJECT_PARENT_FORCE_POINTER,
					NULL, NULL);
		...
		GICallableInfo *argci, void **args)
{
	...
	/* Check 'nil' in optional case. In this case, return NULL as callback. */
	if (lua_isnoneornil(L, narg)) {
		if (optional) {
			*callback = NULL;

			/* Also set associated destroy handler to NULL, because some callees tend to call it when left as garbage even when main callback is NULL(gtk_menu_popup_for_device() case). */
			if (ai != NULL) {
				guint arg;
	...
	if (argci != NULL) {
		...
		if (gi_arg_info_get_closure_index(ai, &arg) && arg <(guint)nargs) {
			...
			if (gi_arg_info_get_destroy_index(ai, &arg) && arg <(guint)nargs)
				((GIArgument *) args[arg])->v_pointer =
					lua_gobject_closure_destroy;
		}
	}

	scope = gi_arg_info_get_scope(ai);
	if (user_data == NULL) {
		/* Closure without user_data block. Create new data block, setup destruction according to scope. */
		...
		int parent, GICallableInfo *ci, void **args)
{
	...
	gboolean optional = (parent == LUA_GOBJECT_PARENT_CALLER_ALLOC) ||
		(ai == NULL || (gi_arg_info_is_optional(ai) || gi_arg_info_may_be_null(ai)));
	...
	case GI_TYPE_TAG_DOUBLE:
	{
		...
		g_return_val_if_fail(parent != LUA_GOBJECT_PARENT_FORCE_POINTER,
			0);
	...
	case GI_TYPE_TAG_FILENAME:
	{
		...
		else if (!optional ||(type != LUA_TNIL && type != LUA_TNONE))
		{
			if (type == LUA_TUSERDATA)
				str = (gchar *) lua_gobject_udata_test(L, narg,
					LUA_GOBJECT_BYTES_BUFFER);
			if (str == NULL)
	...
	case GI_TYPE_TAG_INTERFACE:
	{
		...
		if (GI_IS_ENUM_INFO(info) || GI_IS_FLAGS_INFO(info)) {
			...
			marshal_2c_int(L,
				gi_enum_info_get_storage_type(GI_ENUM_INFO(info)),
				arg, narg, optional, parent);

			/* Remove the temporary value, to keep stack balanced. */
			if (narg == -1)
				lua_pop(L, 1);
		} else if (GI_IS_STRUCT_INFO(info) || GI_IS_UNION_INFO(info)) {
			/* Ideally the gi_type_info_is_pointer() should be sufficient here, but there is some gobject-introspection quirk that some struct arguments might not be marked as pointers (e.g. g_variant_equals(), which has ctype of gconstpointer, and thus logic in girparser.c which sets is_pointer attribute fails). Workaround it by checking also argument type - structs as C function arguments are always passed as pointers. */
			gboolean by_value =
				parent != LUA_GOBJECT_PARENT_FORCE_POINTER &&
			...
			nret = marshal_2c_callable(L, GI_CALLABLE_INFO(info), ai,
				&arg->v_pointer, narg, optional, ci, args);
		} else {
			g_assert_not_reached();
		}
		...
		if (gi_type_info_is_pointer(ti)) {
			...
			if (lua_type(L, narg) == LUA_TSTRING)
				...
				if (type == LUA_TLIGHTUSERDATA)
					...
					/* Check memory buffer. */
					arg->v_pointer = lua_gobject_udata_test(L, narg,
						LUA_GOBJECT_BYTES_BUFFER);
					if (!arg->v_pointer) {
						/* Check object. */
						arg->v_pointer = lua_gobject_object_2c(L, narg,
							G_TYPE_INVALID, FALSE, TRUE, FALSE);
						if (!arg->v_pointer) {
							/* Check any kind of record. */
							lua_pushnil(L);
							lua_gobject_record_2c(L, narg, &arg->v_pointer,
	...
	case GI_TYPE_TAG_ARRAY:
	{
		if (gi_type_info_get_array_type(ti) == GI_ARRAY_TYPE_C) {
			...
			if (pos == 0) {
				...
				/* Allocate underlying array. It is temporary, existing only for the duration of the call. */
				array_guard =
				...
				*array_guard = g_array_sized_new(FALSE, FALSE, elt_size, size);
				...
				/* Convert the allocated array into Lua table with contents. We have to do it in-place. */

				/* Make sure that pos is absolute, so that stack shuffling below does not change the element it points to. */
				if (pos < 0)
					pos += lua_gettop(L) + 1;

				/* Get GArray from the guard and unmarshal it as a full GArray into Lua. */
				...
				/* Deactivate old guard, everything was marshalled into the newly created and marshalled table. */
				*array_guard = NULL;
...
/* Marshalls single value from GLib/C to Lua.  Returns 1 if something was pushed to the stack. */
void
		...
		int parent_arg, int field_arg, int val_arg)
{
	...
	if (lua_gobject_udata_test(L, field_arg, LUA_GOBJECT_GI_INFO)) {
		...
		if ((flags &(getmode ? GI_FIELD_IS_READABLE
			...
			/* Check, whether parent did not disable access checks completely. */
		...
		/* Map GIArgument to proper memory location, get typeinfo of the field and perform actual marshalling. */
		...
		/* Load type information from the table and decide how to handle it according to 'kind' */
		...
		case 3:
		{
			...
			if (getmode) {
				...
				lua_gobject_marshal_2c(L, ti, NULL,
					GI_TRANSFER_NOTHING, field_addr,
					val_arg, 0, NULL, NULL);
				lua_pop(L, 2);
				return 0;
			}
	...
	GICallableInfo *ci = GI_IS_CALLABLE_INFO(pi)
		? GI_CALLABLE_INFO(pi) : NULL;
	if (getmode) {
		lua_gobject_marshal_2lua(L, ti, NULL, GI_DIRECTION_OUT,
			GI_TRANSFER_NOTHING, field_addr, parent_arg, ci, object);
		nret = 1;
	} else {
		lua_gobject_marshal_2c(L, ti, NULL, GI_TRANSFER_EVERYTHING,
			field_addr, val_arg, 0, NULL, NULL);
		nret = 0;
	}
...
/* Creates container(array, list, slist, hash) marshaller for specified container typeinfo. Signature is: marshaller = marshal.container(typeinfo, transfer) */
static int
...
/* Creates marshaller closure for specified fundamental object type. If specified object does not have custom setvalue/getvalue functions registered, returns nil. Signature is: marshaller = marshal.fundamental(gtype) */
static int
marshal_fundamental(lua_State *L)
{
	...
	if (info) {
		...
		if (GI_IS_OBJECT_INFO(info) &&
				gi_object_info_get_fundamental(GI_OBJECT_INFO(info))) {
			GIObjectInfoGetValueFunction get_value =
...
/* Creates or marshalls content of GIArgument to/from lua according to specified typeinfo.
...
/* Workaround for incorrectly annotated g_closure_invoke. Since it is pretty performance-sensitive, it is implemented here in native code instead of creating overlay with custom ffi for it. */
static int
...
/* This is workaround for missing glib function, which should look like this:

void g_closure_set_marshal_with_data(
	GClosure *closure,
